...
// ==========================================
function renderJadwalDashboardDOM(jadwalData, container) {
    container.innerHTML = '';
    const namaHari = ["Minggu", "Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu"];
    const hariIni = namaHari[new Date().getDay()];
    const jadwalHariIni = jadwalData.filter(item => item.Hari === hariIni);
...
async function loadJadwal() {
    ...
    // Tampilkan data dari Cache instan
    const cached = localStorage.getItem('jurnal_jadwal');
    if (cached) {
        try {
            renderJadwalDashboardDOM(JSON.parse(cached), container);
    ...
    try {
        ...
        if (result.status === "success" && result.data.length > 0) {
            localStorage.setItem('jurnal_jadwal', JSON.stringify(result.data));
            renderJadwalDashboardDOM(result.data, container);
        } else {
        ...
        if (loader) loader.classList.add('hidden');
        if (!cached && errorMsg) errorMsg.classList.remove('hidden');
    }
}

function renderJadwalMingguanDOM(jadwalData, container) {
...
async function loadJadwalMingguan() {
    ...
    if (cached) {
        try {
            renderJadwalMingguanDOM(JSON.parse(cached), container);
        } catch (e) { console.error(e); }
    }
    ...
    try {
        ...
        if (result.status === "success" && result.data.length > 0) {
            localStorage.setItem('jurnal_jadwal', JSON.stringify(result.data));
            renderJadwalMingguanDOM(result.data, container);
        }
    } catch (err) {
        console.error(err);
        if (loader) loader.classList.add('hidden');
...
function renderProfilDOM(profil) {
...
async function loadProfil() {
    ...
    const cached = localStorage.getItem('jurnal_profil');
    if (cached) {
        try {
            renderProfilDOM(JSON.parse(cached));
        } catch (e) { console.error(e); }
    }
    ...
    try {
        ...
        if (result.status === "success" && result.data) {
            localStorage.setItem('jurnal_profil', JSON.stringify(result.data));
            renderProfilDOM(result.data);
        }
    } catch (error) {
